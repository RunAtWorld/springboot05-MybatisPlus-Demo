...
public class JacksonUtils {
    ...
    /**
     * 从 JsonNode 下获取 ArrayNode
     *
     * @param node
     * @param arrayKey
     * @return
     */
    public static ArrayNode getArrayNode(JsonNode node, String arrayKey) {
        if (node == null) {
            return null;
        }
        JsonNode arrayNode = node.path(arrayKey);
        if (!arrayNode.isArray()) {
            logger.error("{} is not valid array node", arrayKey);
            return null;
        }
     ...
     * @param node2
     */
    public static void mergeArrayNode(ArrayNode mergedNode, ArrayNode node1, ArrayNode node2) {
        if (mergedNode == null) {
            return;
        }
